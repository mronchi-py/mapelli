...
ALIVE, DEAD = "red", "white" # Colori per gli stati della cella viva e morta
SIZE = 20 # Dimensione della cella in pixel
DELAY = 1000 # Intervallo in millisecondi tra uno step e il successivo
    ...
    for x in range(N):
        for y in range(N):
            canvas.create_rectangle(x*SIZE, y*SIZE, x*SIZE + SIZE, y*SIZE + SIZE,
                                    width=1, outline="lightgrey", fill=matrix[x][y])
...
def transition(matrix):
    ...
    for x in range(N):
        for y in range(N):
            ...
            elif n_neighbors == 3: # Sopravvive o nasce (se era morta) se ha 3 vicini
...
if __name__ == "__main__":    
    matrix = init_matrix(n=200) # Inizializza la matrice con 200 celle ALIVE
    root = tkinter.Tk()
